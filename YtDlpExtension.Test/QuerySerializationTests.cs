...
namespace YtDlpExtension.Test
{
    ...
    public class QuerySerializationTests
    {
        ...
        {
            string? jsonResult = await _ytDlp.TryExecuteQueryAsync("http://abcnews.go.com/ThisWeek/video/week-exclusive-irans-foreign-minister-zarif-20411932");
        ...
        {
            var jsonResult = await _ytDlp.TryExecuteQueryAsync("https://www.youtube.com/watch?v=D-h6MoF7HLA&list=PLXqeB_d1wZEAP6QXjEeHaPbUWvp0EpdqT");
            JObject videoInfo = JObject.Parse(jsonResult);
            VideoData? videoData;
            try
            {
        ...
        {
            var jsonResult = await _ytDlp.TryExecuteQueryAsync("https://www.youtube.com/watch?v=WsDyRAPFBC8");
            JObject videoInfo = JObject.Parse(jsonResult);
            VideoData? videoData;
            try
            {
        ...
        {
            var jsonResult = await _ytDlp.TryExecuteQueryAsync("https://x.com/TumultoBRacervo/status/1923115324771041604");
            JObject videoInfo = JObject.Parse(jsonResult);
            VideoData? videoData;
            try
            {
        ...
        {
            var jsonResult = await _ytDlp.TryExecuteQueryAsync("https://ok.ru/video/230863342321");
            JObject videoInfo = JObject.Parse(jsonResult);
            VideoData? videoData;
            try
            {
        ...
        {
            var jsonResult = await _ytDlp.TryExecuteQueryAsync("https://about.instagram.com/pt-br/features/reels");
            VideoData? videoData;
            try
            {
                videoData = System.Text.Json.JsonSerializer.Deserialize(jsonResult, VideoDataContext.Default.VideoData);
        ...
        {
            var jsonResult = await _ytDlp.TryExecuteQueryAsync("https://www.twitch.tv/gaules");
            VideoData? videoData;
            try
            {
                videoData = System.Text.Json.JsonSerializer.Deserialize(jsonResult, VideoDataContext.Default.VideoData);
